...
class PostRepositoryImpl extends PostRepository {
  ...
  Future<Either<Failure, PostModel>> getPost(
    ...
    if (await networkInfo.isConnected!) {
      try {
        final PostModel remotePost = await remoteDataSource.getPost(params);
  ...
  Future<Either<Failure, List<PostEntity>>> getAllPost() async {
    if (await networkInfo.isConnected!) {
      try {
        ...
        localDataSource.cacheAllPosts(remotePost);
        return Right(remotePost);
      } on ServerException catch (e) {
        return Left(Failure(errMessage: e.errorModel.errorMessage));
      }
      ...
      try {
        final localPost = await localDataSource.getLastAllPosts();
        return Right(localPost);
      } on CacheExeption catch (e) {
        return Left(Failure(errMessage: e.errorMessage));
      }
